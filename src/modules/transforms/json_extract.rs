...
mod tests {
    ...
    async fn test_json_extract_nested_field() {
        ...
        let json_data = vec![
            r#"{"meta": {"req": {"headers": {"dd-device-id": "device-123"}}}}"#,
            r#"{"meta": {"req": {"headers": {"dd-device-id": "device-456"}}}}"#,
        ];
        ...
        config.insert(
            "path".to_string(),
            toml::Value::String("meta.req.headers.dd-device-id".to_string()),
        );
        config.insert(
        ...
        assert!(result_df.column("device_id").is_ok());
        let device_ids = result_df.column("device_id").unwrap().str().unwrap();
        assert_eq!(device_ids.get(0), Some("device-123"));
        assert_eq!(device_ids.get(1), Some("device-456"));
    }
    ...
    async fn test_json_extract_missing_field() {
        ...
        let device_ids = result_df.column("device_id").unwrap().str().unwrap();
        assert_eq!(device_ids.get(0), None);
        assert_eq!(device_ids.get(1), None);
    }
}
